...
// esta funcion realiza una iteracion segun lo definido en LOOP_ITERATIONS_PER_MILLISEC
void burn_1millisecs() {
	unsigned long long i;
	for(i=0; i<LOOP_ITERATIONS_PER_MILLISEC; i++);
}

// realiza un delay segun los miliseconds de la entrada
...
// asigna valores de casio_id y deadline
...
// imprime los valores de process id y deadline
...
// limpia todas las senales del timer para que su valor sea reiniciado
...
// permite cambiar los valores de it_value.tv_sec y it_value.tv_usec
...
// imprimir los valores de las señales del timer
...
// Regresa el tiempo asignado a una task
...
// imprime el id del task que finalizo
void end_task(int s)
{
	printf("\nTask(%d) has finished\n",casio_id);
	exit(0);
